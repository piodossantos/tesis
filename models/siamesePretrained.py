...
class SiamesePretrained(Model):
    model = None
    @staticmethod
    def get_instance(*args):
        if not SiamesePretrained.model:
            SiamesePretrained.model = SiamesePretrained(*args)
        return SiamesePretrained.model

    def __init__(self, *args):
        ck = torch.load('/Users/pio/postgrado/tesis/tesis/screensim-resnet-uda+web350k.ckpt', map_location=torch.device('cpu'))
        self.model = UIScreenEmbedder()
        self.model.load_state_dict(ck['state_dict'])
        self.model = torch.nn.Sequential(*list(self.model.children())[:-1])
        self.model = self.model.eval()
        super().__init__(self.model,"Pretrained Siamese Ne2",args)

    
    def get_embedding(self, image):
        with torch.no_grad():
            return torch.Tensor(self.model(image).flatten())
